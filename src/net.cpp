    ...
    loop
    {
        ...
        else if (nBytes <= 0)
        {
            ...
            else
            {
                // socket error
                int nErr = WSAGetLastError();
                if (fDebug) { printf("recv failed: %d\n", nErr); }
                return false;
            }
        }
    }
...
void ThreadGetMyExternalIP()
{
    ...
    {
        if (fDebug) { printf("GetMyExternalIP() returned %s\n", addrLocalHost.ToStringIP().c_str()); }
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            if (hSocket != INVALID_SOCKET)
                if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr))
                    if (fDebug) { printf("Warning: Unknown socket family\n"); }
            ...
            if (hSocket == INVALID_SOCKET)
            {
                ...
                if (nErr != WSAEWOULDBLOCK)
                    if (fDebug) { printf("socket error accept failed: %d\n", nErr); }
            }
            ...
            {
                if (fDebug) { printf("connection from %s dropped (banned)\n", addr.ToString().c_str()); }
                closesocket(hSocket);
            }
            else
            {
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Deseret " + FormatFullVersion();
        ...
        catch (boost::thread_interrupted)
        {
            ...
            if (fDebug) { printf("UPNP_DeletePortMapping() returned : %d\n", r); }
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        if (addrman.size() == 0 && (GetTime() - nStart > 60)) {
            ...
            if (!done) {
                if (fDebug) { printf("Adding fixed seed nodes as DNS doesn't seem to be available.\n"); }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Deseret is probably already running."), addrBind.ToString().c_str());
        else
        ...
        BOOST_FOREACH(SOCKET hListenSocket, vhListenSocket)
            if (hListenSocket != INVALID_SOCKET)
                if (closesocket(hListenSocket) == SOCKET_ERROR)
                    if (fDebug) { printf("closesocket(hListenSocket) failed with error %d\n", WSAGetLastError()); }
